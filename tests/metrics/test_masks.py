...
def test_cli_gen_manual_depth_with_pattern_misaligned_frame_ids_raises(
    tmp_path: Path,
) -> None:
    """Pattern discovery must reject same-length inputs with mismatched frame ids."""
    imageio = pytest.importorskip("imageio.v3")
    ...
    assert not (out_dir / "occlusion_mask.npy").exists()
