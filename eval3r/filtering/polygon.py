"""Open3D SelectionPolygonVolume filter (extruded-polygon prisms).
...
@dataclass
class PolygonFilter(BaseFilter):
    ...
    @classmethod
    def load_from_json(cls, path: PathLike) -> PolygonFilter:
...
def crop_points_inside(volume: PolygonFilter, points: np.ndarray) -> np.ndarray:
...
def load_crop_volume_json(path: PathLike) -> PolygonFilter:
    """Parse an Open3D ``SelectionPolygonVolume`` JSON file.

    Thin wrapper around :meth:`PolygonFilter.load_from_json`.
    """
    return PolygonFilter.load_from_json(path)


__all__ = ["PolygonFilter", "load_crop_volume_json", "crop_points_inside"]
