...
from eval3r.presets.eth3d import ETH3D_PRESET


def command(
    pred_root: str = typer.Option(..., "--pred-root", help="Predictions root."),
    gt_root: str = typer.Option(..., "--gt-root", help="ETH3D dataset root."),
    split: str | None = typer.Option(None, "--split", help="Scene-id list."),
    pred_pattern: list[str] = typer.Option([], "--pred-pattern"),
    track: str = typer.Option("dslr", "--track"),
    mesh_filename: str = typer.Option("scan.ply", "--mesh-filename"),
    point_cloud_filename: str = typer.Option("scan_points.ply", "--point-cloud-filename"),
    ...
    ds = ETH3DAdapter(gt_root, split=split, validate_on_init=False, **adapter_kwargs)
    cfg = build_config(
        ETH3D_PRESET,
