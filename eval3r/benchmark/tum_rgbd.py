"""TUM RGB-D benchmark CLI wiring."""
...
from eval3r.presets.tum_rgbd import TUM_RGBD_PRESET


def command(
    pred_root: str = typer.Option(..., "--pred-root", help="Predictions root."),
    gt_root: str = typer.Option(..., "--gt-root", help="TUM RGB-D dataset root."),
    split: str | None = typer.Option(None, "--split", help="Scene-id list."),
    pred_pattern: list[str] = typer.Option([], "--pred-pattern"),
    rgb_subdir: str = typer.Option("rgb", "--rgb-subdir"),
    depth_subdir: str = typer.Option("depth", "--depth-subdir"),
    color_format: str = typer.Option("{ts:.6f}.png", "--color-format"),
    depth_format: str = typer.Option("{ts:.6f}.png", "--depth-format"),
    depth_scale: float = typer.Option(5000.0, "--depth-scale"),
    pose_filename: str = typer.Option("groundtruth.txt", "--pose-filename"),
    associations_file: str | None = typer.Option(None, "--associations-file"),
    ...
    ds = TumRGBDAdapter(gt_root, split=split, validate_on_init=False, **adapter_kwargs)
    cfg = build_config(
        TUM_RGBD_PRESET,
